#!/usr/bin/env bash
#
# release.sh — bump build, archive, and upload Open Feelings to TestFlight.
#
# Usage:
#   ./scripts/release.sh                # default: bump build only
#   ./scripts/release.sh --patch        # also bump patch (1.0.1 → 1.0.2)
#   ./scripts/release.sh --minor        # also bump minor (1.0.1 → 1.1.0)
...
OLD_BUILD=$(awk '/^[[:space:]]*CURRENT_PROJECT_VERSION:/ {print $2; exit}' "$PROJECT_YML")
[[ -n "$OLD_BUILD" ]] || fail "Could not read CURRENT_PROJECT_VERSION from project.yml"

OLD_VERSION=$(awk '/^[[:space:]]*MARKETING_VERSION:/ {print $2; exit}' "$PROJECT_YML")
...
# ---------- archive ----------
ARCHIVE_PATH="/tmp/OpenFeelings-build${NEW_BUILD}.xcarchive"
EXPORT_PATH="/tmp/OpenFeelings-build${NEW_BUILD}-export"
PROJECT="$REPO_ROOT/OpenFeelings.xcodeproj"
SCHEME="OpenFeelings"
EXPORT_OPTIONS="$REPO_ROOT/OpenFeelings/ExportOptions.plist"
...
xcodebuild \
    ...
    -authenticationKeyIssuerID "$ASC_ISSUER" 2>&1 | tee /tmp/openfeelings-export.log | grep -E "Export Succeeded|EXPORT SUCCEEDED|error:|\*\*" | head -10

if ! grep -q "EXPORT SUCCEEDED" /tmp/openfeelings-export.log; then
    fail "Export failed — see /tmp/openfeelings-export.log"
fi
...
echo -e "\n${GREEN}✔ Open Feelings $NEW_VERSION (build $NEW_BUILD) uploaded to TestFlight${NC}"
