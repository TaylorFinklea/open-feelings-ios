...
- [ ] **Natural-language check-in entry (Phase 1)** — spec + plan written 2026-06-26 on branch `feat/natural-language-entry` (off `main`; **not implemented yet**). Plain-language entry on two surfaces (in-app `QuickEntryView` + Siri `LogFeelingIntent`) feeding one `FeelingParser` seam → deterministic `KeywordFeelingParser` (also the permanent fallback). Core fields only (emotion + 1–5 intensity + note); stop-at-best-core; fully on-device. Decisions: Siri `.none` → opens app pre-filled (`PendingQuickEntryStore`), in-app entries `captureSource="quickentry"`, synonym table original-MIT. **Landmines baked into the plan**: (1) NO shared `ModelContainer` exists — `makeModelContainer()` builds a fresh one per call; Task 5 hoists to `OpenFeelingsModelContainer.shared` (a 2nd container over the CloudKit store = corruption). (2) `CheckInView.save()` sets `customBodyRegionIDs` at :254 *outside* the constructor — the `FeelingLogService` extraction must carry it. (3) AppIntents conditional app-open (`opensIntent:`) is the one verify-against-docs API (Task 11). 13 TDD tasks. Spec + plan at `docs/superpowers/{specs,plans}/2026-06-26-natural-language-entry*.md`. **Phase 2** (Apple FoundationModels behind the same protocol) is a separate later plan.
