...
- [ ] **Tip jar (in-app donations)** — decided 2026-05-27 (product round). In-app tip jar via Apple IAP, **pure donation** (unlocks nothing, everything stays free). Consumable/repeatable. Three tiers: **Soda $1.99 / Lunch $4.99 / Dinner $9.99**. Build as a reusable module — generic `StoreKitClient` + app-specific `TipJarService` — so it ports to other projects. New `Support Open Feelings` Settings section + thank-you state. Needs App Store Connect IAP product setup (3 consumables). Not yet specced/planned.
