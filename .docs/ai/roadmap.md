...
- [x] **CloudKit follow-up deploy — add `CD_ThoughtRecord`** — done 2026-05-29. Saved a thought record on a device build to declare the type in Development, then Deployed Schema Changes to Production (additive 1-type + `CD_createdAt` Queryable index). Confirmed the Settings → Privacy iCloud-sync `CKErrorDomain error 2` (partialFailure) cleared. **All 8 record types now in Production.**
...
- [ ] **Per-surface CRUD arc** (user wants to view/edit/delete all their data). Decided 2026-05-28; **per-surface** (not a unified console), and **check-ins become fully editable** (reverses "moment not journal" — user-approved).
  ...
  - [x] **A — Check-in full edit** — done 2026-05-28. Tap a History entry's Edit → `CheckInEditView` (reuses the wizard step views) edits every field; `createdAt`/`id`/`healthSyncStatus`/`captureSource` preserved. `CheckInDraft.from(log:)`/`apply(to:)` + 16 unit tests + 1 UI test. Note-only edit + `NoteEditorSheet` removed. Spec at `docs/superpowers/specs/2026-05-28-checkin-full-edit-design.md`. **Per-surface CRUD arc complete (A–F).**
- [ ] **Follow-up: History (+ ThoughtRecordsArea) `.swipeActions` are non-functional** — they sit in a ScrollView+VStack, not a List, so swipe-to-delete silently doesn't fire; only the VoiceOver `.accessibilityAction(named: "Delete")` works. Surfaced by the CRUD adversarial reviews. Fix by converting those lists to `List` or adding an explicit affordance (mirror CustomValuesSheet). Small.
...
- [ ] **Natural-language check-in entry (Phase 1)** — **implemented 2026-06-26** on branch `feat/natural-language-entry` (off `main`; **not merged, not pushed**; 15 commits after the plan). Plain-language entry on two surfaces (in-app `QuickEntryView` + Siri `LogFeelingIntent`) feeding one `FeelingParser` seam → deterministic `KeywordFeelingParser` (permanent fallback). Core fields only (emotion + 1–5 intensity + note); stop-at-best-core; fully on-device. Siri `.none` → opens app pre-filled (`PendingQuickEntryStore`); in-app entries `captureSource="quickentry"`, Siri `"siri"`. **Verified**: full unit suite + QuickEntry/wizard UI `** TEST SUCCEEDED **`; 4-lens adversarial review (save-refactor + container hoist + Siri integration all sound) caught 3 parser bugs (token-aware intensity + (trust,depth) tiebreak) — fixed + 5 regression tests. Implementation decisions in `decisions.md` 2026-06-26. Spec + plan at `docs/superpowers/{specs,plans}/2026-06-26-natural-language-entry*.md`.
  - [x] **In-app flow verified** — driven live in the iPhone 16 sim (idb) 2026-06-27: Today → "Say how you feel" → type → Continue → Review (Fearful>Anxious, 4/5 dots, note) → Save → entry on Today with quick-entry glyph; parser-fix probe ("everyone…anxious" → no intensity) PASS.
  - [x] **Shipped to TestFlight as 1.0.1 (build 44)** 2026-06-27 (commit `a383ad9`). **Marketing version bumped 1.0 → 1.0.1 — the 1.0 train is closed (1.0 approved on ASC); all future builds are 1.0.x.** See `decisions.md` 2026-06-27.
  - [x] **Merged to `main`** 2026-07-11 (fast-forward, via the watch branch superset). **Shipped as 1.0.1 build 45.**
  - [ ] **User testing on TestFlight** (bd `iuq`), esp. the **Siri/App Shortcut** path (real device only): "Log a feeling in Open Feelings" — high-confidence saves a `siri` log + speaks read-back; unrecognizable input opens the app to QuickEntry pre-filled.
  - [ ] **Phase 2** (separate later plan): `FoundationModelsFeelingParser` behind the same `FeelingParser` protocol, availability-gated, keyword fallback. iOS 26 already supports it — no deployment bump.

- [ ] **Watch natural-language entry** — **implemented 2026-06-29** on branch `feat/watch-natural-language-entry` (**stacked on `feat/natural-language-entry`**; not merged; ~13 commits). Reuses the iOS `KeywordFeelingParser` on watchOS. Two surfaces: in-app dictation (mic on `CheckInRootView` → `DictationEntryView` → parse → confirm/picker → existing `WatchCheckInPayload` send) + watch Siri Shortcut (`WatchCheckInIntent`). `WatchSessionClient.shared` hoist; `PendingWatchEntryStore` for the Siri `.none` hand-off; `captureSource` stays `"watch"`. **Verified**: clean watch build + 24 watch tests + 20 iOS-regression tests green; adversarial review clean (its one "blocker" was a false positive — see `decisions.md` 2026-06-29). Spec + plan at `docs/superpowers/{specs,plans}/2026-06-29-watch-natural-language-entry*.md`.
  - [x] **Merged to `main`** 2026-07-11 (fast-forward) and **shipped as 1.0.1 build 45** (single build carries both iOS + watch NL entry, since the watch app embeds in the iOS app).
  - [ ] **Manual device checks** (bd `q6k`; not sim-verifiable — real paired watch): in-app mic → dictate → confirm/picker → Send → entry reaches the phone ("From Apple Watch"); "I'm anxious" (no intensity) routes through IntensityPicker; gibberish → core picker with note. Watch Siri "Log a feeling in Open Feelings" sends; unrecognizable input opens the watch app pre-seeded. Offline send queues + drains.
  - Watch sim quirk (for future builds): the `name=OF Watch Test` destination fails (OS:latest mismatch); use `-destination 'id=<watch sim id>'` (find via `xcrun simctl list devices available | grep -i watch`).
