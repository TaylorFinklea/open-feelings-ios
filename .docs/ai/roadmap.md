...
- [ ] **Natural-language check-in entry (Phase 1)** — **implemented 2026-06-26** on branch `feat/natural-language-entry` (off `main`; **not merged, not pushed**; 15 commits after the plan). Plain-language entry on two surfaces (in-app `QuickEntryView` + Siri `LogFeelingIntent`) feeding one `FeelingParser` seam → deterministic `KeywordFeelingParser` (permanent fallback). Core fields only (emotion + 1–5 intensity + note); stop-at-best-core; fully on-device. Siri `.none` → opens app pre-filled (`PendingQuickEntryStore`); in-app entries `captureSource="quickentry"`, Siri `"siri"`. **Verified**: full unit suite + QuickEntry/wizard UI `** TEST SUCCEEDED **`; 4-lens adversarial review (save-refactor + container hoist + Siri integration all sound) caught 3 parser bugs (token-aware intensity + (trust,depth) tiebreak) — fixed + 5 regression tests. Implementation decisions in `decisions.md` 2026-06-26. Spec + plan at `docs/superpowers/{specs,plans}/2026-06-26-natural-language-entry*.md`.
  ...
  - [ ] **User testing on TestFlight**, esp. the **Siri/App Shortcut** path (only testable on a real device with Shortcuts): "Log a feeling in Open Feelings" — high-confidence saves a `siri` log + speaks read-back; unrecognizable input opens the app to QuickEntry pre-filled. Then **merge `feat/natural-language-entry` → main**.
...
- [ ] **Watch natural-language entry** — **implemented 2026-06-29** on branch `feat/watch-natural-language-entry` (**stacked on `feat/natural-language-entry`**; not merged; ~13 commits). Reuses the iOS `KeywordFeelingParser` on watchOS. Two surfaces: in-app dictation (mic on `CheckInRootView` → `DictationEntryView` → parse → confirm/picker → existing `WatchCheckInPayload` send) + watch Siri Shortcut (`WatchCheckInIntent`). `WatchSessionClient.shared` hoist; `PendingWatchEntryStore` for the Siri `.none` hand-off; `captureSource` stays `"watch"`. **Verified**: clean watch build + 24 watch tests + 20 iOS-regression tests green; adversarial review clean (its one "blocker" was a false positive — see `decisions.md` 2026-06-29). Spec + plan at `docs/superpowers/{specs,plans}/2026-06-29-watch-natural-language-entry*.md`.
  - [ ] **Manual device checks** (not sim-verifiable — real paired watch): in-app mic → dictate → confirm/picker → Send → entry reaches the phone ("From Apple Watch"); "I'm anxious" (no intensity) routes through IntensityPicker; gibberish → core picker with note. Watch Siri "Log a feeling in Open Feelings" sends; unrecognizable input opens the watch app pre-seeded. Offline send queues + drains.
  - [ ] **Merge order**: `feat/natural-language-entry` → `main` FIRST (it carries the parser), then `feat/watch-natural-language-entry` → `main`. **Watch sim quirk**: use `-destination 'id=0E0A86BF-E00C-4E3B-94E7-D086FEE62608'` (the `name=OF Watch Test` form fails — OS:latest mismatch).
